...
int main(int argc, char *argv[]) {
    ...
    recommender = new EASEppRecommender(
            train.max_user() + 1,
            train.max_item() + 1,
    ...
    ((EASEppRecommender *) recommender)->SetPrintTrainStats(
    ...
    auto evaluate = [&](int epoch) {
        ...
        if (epoch == num_epochs) {
            Recommender::writeOutput(flags["results_file"], "easepp", flags, metrics, train_time);
        }
    };
