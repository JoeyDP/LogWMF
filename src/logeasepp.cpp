...
const VectorXf ProjectLogEASEBlock(
    ...
    MatrixXf matrix = 2 * unobserved_weight * local_gramian;
    ...
    for (const auto &item_and_rating_index: user_history) {
        ...
        const VectorSpf cp_v = local_item_embedding.row(cp);
...
LogEASEppRecommender::LogEASEppRecommender(int num_users, int num_items, float reg,
...
VectorXf LogEASEppRecommender::EvaluateDataset(const Dataset &data, const SpMatrix &eval_by_user) {
    // Evalute the dataset.
...
void LogEASEppRecommender::ComputeLosses(const Dataset &data, const VectorXf &prediction) {
...
void LogEASEppRecommender::Train(const Dataset &data) {
    // Set user embedding
...
template<typename F>
void LogEASEppRecommender::Step(const SpMatrix &data_by_user,
    ...
    std::mutex m;
    auto Project = ProjectLogEASEBlock;
