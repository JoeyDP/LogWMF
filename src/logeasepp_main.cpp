...
int main(int argc, char* argv[]) {
    ...
    recommender = new LogEASEppRecommender(
            train.max_user()+1,
            train.max_item()+1,
    ...
    ((LogEASEppRecommender*)recommender)->SetPrintTrainStats(
    ...
    auto evaluate = [&](int epoch) {
        ...
        if (epoch == num_epochs) {
            Recommender::writeOutput(flags["results_file"], "logeasepp", flags, metrics, train_time);
        }
    };
